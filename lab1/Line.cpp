...
void mouseButtonCallback(GLFWwindow* window, int button, int action, int mods) {
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
        ...
        if (firstPoint) {
            ...
            std::cout << "Drawing line using " << (algorithmType == 0 ? "Bresenham" : "Midpoint") << " algorithm" << std::endl;
        }
    }
}
...
void render() {
    ...
    // Vẽ đoạn thẳng nếu đã có đủ 2 điểm
    if (points.size() >= 2) {
        ...
        if (algorithmType == 0) {
            ...
            midpointLine(points[0].x, points[0].y, points[1].x, points[1].y);
        }
    }
...
int main() {
    ...
    // Tạo cửa sổ
    window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Line Drawing Algorithms", NULL, NULL);
    if (!window) {
    ...
    std::cout << "Line Drawing Program" << std::endl;
    std::cout << "----------------------" << std::endl;
    std::cout << "Instructions:" << std::endl;
    std::cout << "1. First click sets the first point of the line" << std::endl;
    std::cout << "2. Second click sets the second point and draws the line" << std::endl;
    std::cout << "3. Press '1' or click '1' button to use Bresenham algorithm" << std::endl;
    std::cout << "4. Press '2' or click '2' button to use Midpoint algorithm" << std::endl;
