...
void mouseButtonCallback(GLFWwindow* window, int button, int action, int mods) {
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
        ...
        if (firstPoint) {
            ...
            std::cout << "Drawing cos(x) from x=" << x_start << " to x=" << x_end << " using " << (algorithmType == 0 ? "Bresenham" : "Midpoint") << " for segments" << std::endl;
        }
    }
}
...
void render() {
    ...
    // Vẽ đường cos(x) nếu đã có đủ 2 điểm
    if (points.size() >= 2) {
        int x_start = std::min(points[0].x, points[1].x);
        int x_end = std::max(points[0].x, points[1].x);
        ...
        int prev_x = x_start;
        double prev_y_d = center_y + amp * cos(freq * (prev_x - x_start));
        int prev_y = static_cast<int>(round(prev_y_d));
        
        for (int x = x_start + 1; x <= x_end; ++x) {
            double y_d = center_y + amp * cos(freq * (x - x_start));
...
int main() {
    ...
    // Tạo cửa sổ
    window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Cos Curve Drawing Algorithms", NULL, NULL);
    if (!window) {
    ...
    std::cout << "Cos Curve Drawing Program" << std::endl;
    std::cout << "---------------------------" << std::endl;
    std::cout << "Instructions:" << std::endl;
    std::cout << "1. First click sets the start x of the cos curve" << std::endl;
    std::cout << "2. Second click sets the end x and draws the cos(x) curve (y-coordinates ignored)" << std::endl;
